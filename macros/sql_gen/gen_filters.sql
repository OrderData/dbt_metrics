...
{%- macro default__gen_filters(model_values, start_date, end_date, where, grain) -%}
    ...
    {% if start_date or end_date %}
        ...
        {% if start_date and end_date -%}
            cast(base_model.{{model_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) >= cast('{{ start_date }}' as date)
            and cast(base_model.{{model_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) < cast('{{ end_date }}' as date)
        {%- elif start_date and not end_date -%}
            cast(base_model.{{model_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) >= cast('{{ start_date }}' as date)
        {%- elif end_date and not start_date -%}
            cast(base_model.{{model_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) < cast('{{ end_date }}' as date)
        {%- endif %} 
        )
    {% endif -%} 
