...
{% macro default__gen_calendar_join(group_values, grain) %}
        ...
        {%- else %}
            {%- if grain in ['hour','minute'] %} on {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} on cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} = calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}
        {% endif -%}
{% endmacro %}
...
{% macro bigquery__gen_calendar_join(group_values, grain) %}
        left join calendar 
        {%- if group_values.window is not none %}
            {%- if grain in ['hour','minute'] %} on {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} on cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} > date_sub(calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}, interval {{group_values.window.count}} {{group_values.window.period}})
            {%- if grain in ['hour','minute'] %} and {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} and cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} < calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}
        {%- else %}
            {%- if grain in ['hour','minute'] %} on {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} on cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} = calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}
        {% endif -%}
...
{% macro postgres__gen_calendar_join(group_values, grain) %}
        left join calendar 
        {%- if group_values.window is not none %}
            {%- if grain in ['hour','minute'] %} on {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} on cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} > calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%} - interval '{{group_values.window.count}} {{group_values.window.period}}'
            {%- if grain in ['hour','minute'] %} and {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} and cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} < calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}
        {%- else %}
            {%- if grain in ['hour','minute'] %} on {{ date_trunc('minute', 'base_model.' + group_values.timestamp | replace( '<<date_grain>>' , grain )) + ' ' }} {%- else -%} on cast(base_model.{{group_values.timestamp | replace( '<<date_grain>>' , grain ) }} as date) {%- endif -%} = calendar.date_{%- if grain in ['hour','minute'] %}minute{%- else -%}day{%- endif -%}
        {% endif -%}
